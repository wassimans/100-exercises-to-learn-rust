...
impl Ticket {
    pub fn new(title: String, description: String, status: Status) -> Result<Ticket, String> {
        ...
        if title.len() > 50 {
            return Err::<Ticket, String>("Title cannot be longer than 50 bytes".to_string());
        }
        if description.is_empty() {
            return Err::<Ticket, String>("Description cannot be empty".to_string());
        }
        if description.len() > 500 {
            return Err::<Ticket, String>(
