...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Err(val) if val == *"Title cannot be empty" => panic!("Title cannot be empty"),
        Err(val) if val == *"Title cannot be longer than 50 bytes" => {
            panic!("Title cannot be longer than 50 bytes")
        }
        Err(val) if val == *"Description cannot be empty" => Ticket::new(
            title.clone(),
            "Description not provided".to_string(),
            status,
        )
        .unwrap(),
        Err(val) if val == *"Description cannot be longer than 500 bytes" => {
